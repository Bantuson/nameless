-- 0005: sqlxmq queue schema (mq_msgs / mq_payloads + poll/ack functions).
--
-- VENDORED verbatim from sqlxmq 0.6.0 (github.com/Diggsey/sqlxmq, tag 0.6.0, MIT/Apache-2.0),
-- concatenating its five up-migrations in timestamp order. The durable JobQueue adapter
-- (queue_sqlxmq.rs) spawns into these tables; without this DDL the queue cannot enqueue and the
-- compile-time-checked mq_msgs query in its cross-restart test does not build (found by CI).
-- Re-vendor if the sqlxmq dependency is ever bumped past 0.6.x.

-- ===== sqlxmq migration: 20210316025847_setup.up.sql =====
...
-- ===== sqlxmq migration: 20210921115907_clear.up.sql =====
...
-- ===== sqlxmq migration: 20211013151757_fix_mq_latest_message.up.sql =====
...
$$ LANGUAGE SQL STABLE;
-- ===== sqlxmq migration: 20220208120856_fix_concurrent_poll.up.sql =====
...
-- ===== sqlxmq migration: 20220713122907_fix-clear_all-keep-nil-message.up.sql =====
